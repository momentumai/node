...
module.exports = function (className) {
    function info () {
        if (levels.indexOf(config.logLevel) === 0) {
            console.log([n(), 'INFO', className, parse(arguments)].join('|'));
        }
    }

    function debug () {
        if (levels.indexOf(config.logLevel) < 2) {
            console.log([n(), 'DEBUG', className, parse(arguments)].join('|'));
        }
    }

    function warn () {
        if (levels.indexOf(config.logLevel) < 3) {
            console.log([n(), 'WARN', className, parse(arguments)].join('|'));
        }
    }

    function error () {
        if (levels.indexOf(config.logLevel) < 4) {
            console.log([n(), 'ERROR', className, parse(arguments)].join('|'));
        }
    }
