...
function exclude (item, excludeIndexes) {
    var data = [];
...
ds.set = function (namespace, kind, key, item, excludeIndexes) {
    ...
    return (new Promise(function (resolve, reject) {
        datastore.save({
            ...
            'data': exclude(item, excludeIndexes)
        }, function (err) {
            if (err) {
                return reject(err);
            }
        ...
        if (err.code === 503) {
            return ds.set(namespace, kind, key, item);
        }
...
ds.setMulti = function (namespace, kind, data, excludeIndexes) {
    var datastore = getDataStore(namespace);
    ...
    return (new Promise(function (resolve, reject) {
        ...
        data.forEach(function (item) {
            item.key = item.key || uuid.v4();

            dsData.push({
                'key': datastore.key([String(kind), String(item.key)]),
                'data': exclude(item.data, excludeIndexes)
            });
        });
...
ds.delete = function (namespace, kind, keys) {
    var datastore = getDataStore(namespace);
    ...
    return (new Promise(function (resolve, reject) {
        ...
        if (err.code === 503) {
            return ds.delete(namespace, kind, keys);
        }
