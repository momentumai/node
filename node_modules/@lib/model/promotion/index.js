...
function getSavedAudience (audience, teamId) {
    return audienceModel.get(teamId, audience).then(function (item) {
        return item.data || {};
    });
}

function createAdset (campaignName, campaignId, settings, fb, teamId) {
...
promotion.stopPromotion = function (teamId, adsetId, campaignId, now, pool) {
    return db.q(q['get_token'], {
        ...
        if (!rows[0] || !rows[0].access_token) {
            throw 'model/promotion:400:No crawler token:' + teamId;
        }

        token = rows[0].access_token;

        return Facebook.post(adsetId, params, token);
    }).then(function () {
        return db.q(q['update_promotion'], {
            'id': campaignId,
            'end_time': now
        }, pool);
    });
...
promotion.endPromotion = function (teamId, campaignId, fb, now, pool) {
    return db.q(q['get_promotion'], {
        'team_id': teamId,
        'id': campaignId
    }, pool).then(function (rows) {
