...
function getAdSet (token, adsetId) {
    console.error(adsetId);
    return Facebook.get([
        adsetId,
        '?fields=name,start_time,end_time,effective_status,lifetime_budget,',
...
function getUrls (teamId, ids) {
    ...
    return ds.init(teamId).then(function (datastore) {
        ...
        ids.forEach(function (id) {
            queries[id] = ds.query('Content', function (query) {
                return query.filter('content_id', id);
            }, null, datastore).then(function (res) {
                if (res && res[0] && res[0]) {
    ...
    return db.q(q['list_campaigns'], {
        ...
        Object.keys(adsets).forEach(function (id) {
            var insights = adsets[id].insights &&
                ...
                adset = Object.keys(adsets[id]).length ?
                    adsets[id] : null;

            ret.data[id].adset = adset;

            if (adset) {
                if (adsets[id].ads && adsets[id].ads.data) {
                    ret.data[id].status =
                        adsets[id].ads.data[0].effective_status;
                } else {
                    ret.data[id].status = adsets[id].effective_status;
                }
                ret.data[id].active = (
                    ret.data[id].status === 'ACTIVE' ||
                    ret.data[id].status === 'PREAPPROVED'
                );
                ...
                if (!insights.spend || !ret.data[id].click) {
                    ret.data[id].cpc = -1;
                } else {
                    ret.data[id].cpc = insights.spend / ret.data[id].click;
                }
                ...
                ret.data[id].name = [
                    '[',
                    ret.data[id].meta.adaccount || 'Unknown account',
                    ']'
                ].join('') +
                ' Campaign on ' +
                ret.data[id].meta.page || 'Unknown page';
                ret.data[id].url = null;
            }
        });
...
promotion.listCampaigns = function (context, event, pool) {
    ...
    return Promise.props(promises).then(function (resp) {
        ...
        ret.data.forEach(function (row) {
            ...
            row.cpc = formatMoney(row.cpc, row.meta.currency);
        });
...
promotion.saveCampaign = function (context, event, pool) {
    return db.q(q['get_campaign'], {
        ...
        if (!c) {
            throw 'model/promotion:400:Campaign does not exist';
        }
        ...
        return Facebook.post(
            adsetId,
            {
                ...
                'lifetime_budget': event.budget
            },
            context.fb.token
        );
    }).then(function () {
        return db.q(q['save_campaign'], {
            'id': event.campaign_id,
            'end_time': event.end_time
        }, pool);
    }).then(function () {
...
promotion.export = function (context, event, pool) {
    ...
    return listCampaigns(
        ...
        res.data.filter(function (act) {
            return act['adset'] && act['is_spent'];
            ...
            data.push({
                ...
                'CPC': act.cpc === -1 ? getUndefined() : act.cpc,
                'Spent': act.spend,
                'Click': act.click,
                'Lift': act.lift,
