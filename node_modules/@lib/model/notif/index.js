...
notif.addToken = function (userId, endpoint) {
    ...
    return ds.extend('User', String(userId), function (user) {
...
notif.getToken = function (userId) {
    return ds.get('User', String(userId)).then(function (user) {
        user = user && user.data || {};
        user['registration_ids'] = user['registration_ids'] || [];
...
notif.fetch = function (userId, endpoint) {
    ...
    return notif.validUser(userId, token).then(function (valid) {
        ...
        return ds.query('Notification', function (query) {
            return query.filter('registrationToken', token);
        });
    }).then(function (items) {
        return ds.delete('Notification', items.map(function (item) {
...
notif.remove = function (registrationToken, collapseKey) {
    return ds.query('Notification', function (query) {
        ...
        ds.delete('Notification', items.map(function (item) {
            return item.key.name;
        }));
    });
};
...
notif.send = function (registrationTokens, params, data) {
    ...
    return ds.set('Notification', setData).then(function () {
