...
function parseParams (params) {
    params.team_id = params.team_id || 0;
    params.cats = params.cats || [];
    params.adaccount = params.adaccount || 'NONE';
...
function validateCampaign (campaign, token) {
    var url = campaign.data.fb_id,
        params = {
            'status': 'ACTIVE'
        };

    if (!campaign || !campaign.data || !campaign.data.fb_id) {
...
function getCampaignName (cats, nameText) {
    var name = ['Momentum'],
        pushedCat = false;
...
function add (params, token) {
    ...
    return createFbCampaign(
        getCampaignName(params.cats, params.name_text),
        params.adaccount,
        token
    ).then(function (resp) {
...
campaignModel.addGet = function (params, token) {
    ...
    return campaignModel.get(params, token)
