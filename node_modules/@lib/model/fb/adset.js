...
function parseParams (p) {
    ...
    params.end_time = Math.round(Number(p.end_time)) || 0;
    params.start_time = Math.round(Number(p.start_time)) || 0;
...
adsetModel.getDetails = function (adset, token) {
    var url = [
        adset.name,
        '?fields=lifetime_budget,daily_budget,account_id,',
    ...
    return Facebook.get(url, token).then(function (resp) {
        ...
        return {
            'has_permission': true,
            'adaccount': resp.account_id,
            'fb_actions': Number(insights.total_actions) || 0,
...
function parseUpdateParams (p) {
    ...
    if (typeof p.active !== 'undefined') {
        params.status = p.active ? 'ACTIVE' : 'PAUSED';
    }
    ...
    if (typeof p.end_time !== 'undefined') {
        params.end_time = p.end_time;
    }

    if (typeof p.budget !== 'undefined' &&
        typeof p.budget_type !== 'undefined') {
        params[p.budget_type] = p.budget;
    }

    return params;
}

adsetModel.update = function (adset, params, token) {
