...
function getAdAccounts (adaccounts, token) {
...
function getFbPages (pages, token) {
    return Promise.all(Object.keys(pages).map(function (item) {
        return Facebook.get([item, '?fields=name,picture'].join(''), token)
        .then(function (resp) {
            return {
                'id': item,
                'name': resp.name,
...
module.exports = function (params, items, token) {
    ...
    return Promise.resolve().then(function () {
        ...
        params.info = Object.keys(
            result.info
        ).reduce(function (prev, contentId) {
            prev[contentId] = result.info[contentId];
            return prev;
        }, {});

        return Promise.all([
            getAdAccounts(adaccounts, token),
            getFbPages(pages, token)
        ]);
    }).then(function (results) {
        var currencies = results[0].reduce(function (prev, act) {
                prev[act['adaccount']] = act['currency'];
                return prev;
            }, {}),
            pages = results[1].reduce(function (prev, act) {
                prev[act['id']] = act;
                return prev;
            }, {});
        ...
        return experiments.map(function (item) {
            ...
            extend(ret, {
                'id': item.key.name,
                'name': params.info[item.key.name].title,
                'url': params.info[item.key.name].url,
                ...
                'tests': tests.map(function (test) {
                    ...
                    return {
                        ...
                        'budget_object': {
                            'data': test.budget,
                            'currency': currencies[test.adaccount]
                        },
                        ...
                        'cpa_unformated': divide(test.spend, test.click),
                        'cpc_unformated': divide(test.spend, test.fb_actions)
                    };
                })
            });
