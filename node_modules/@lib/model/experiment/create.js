...
function addTest (test, params, token) {
    return experimentTestCreate({
        ...
        'experiment_id': params.contentInfo.id,
...
module.exports = function (params, token) {
    ...
    .then(function (contentInfo) {
        ...
        params.tests = tests.map(function (item) {
            return ds.jsonToKey(item.key);
        });

        return ds.extend('Experiment', params.contentInfo.id, function (item) {
            return {
                'name_text': params.name_text,
                'cat1': item.cat1 || params.contentInfo.cats[0],
                'cat2': item.cat2 || params.contentInfo.cats[1],
                'cat3': item.cat3 || params.contentInfo.cats[2],
                'tests': (item.tests || []).concat(params.tests),
                'created_at': item.created_at || new Date(),
                'updated_at': new Date(),
                'active': true
            };
        }, params.team_id).then(function () {
            return 'successful';
        });
    });
};
