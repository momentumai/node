...
<html>
  <head>
    ...
    <style type="text/css">
      body { color: #000; font: normal 100% Helvetica, Arial, sans-serif; }
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        {% if account.logo_url != blank %}
          <img src="{{ account.logo_url }}" width="300" />
        {% endif %}
        </td>
        <td valign="bottom">
          <h1>{{title}}</h1>
      ...
      <tr>
        <td width="20%" rowspan="2" valign="top" class="space12">
          ...
          <p class="txt">{{document.due_date | date}}</p>
          {% endif %}
          {% if document.valid_until != blank %}
          <h2 class="label border1">{{labels.valid_until}}</h2>
          <p class="txt">{{document.valid_until | date}}</p>
          {% endif %}
        </td>
        <td width="38%" valign="top" class="space12">
          <h2 class="label border-first">{{labels.from}}</h2>
          <h3 class="txt">{{ account.full_name }}</h3>
          <p>{% if account.tax_id != blank %} {{labels.tax_id}}: {{account.tax_id}} {% endif %}</p>
          <p>{% if account.vat_number != blank %} {{labels.vat_number}}: {{account.vat_number}} {% endif %}</p>
          <p>{% if account.company_number != blank %} {{labels.company_number}}: {{account.company_number}} {% endif %}</p>
          <address>{{ account.formatted_address | newline_to_br }}</address>
          <p><a href="mailto:{{account.email}}">{{ account.email }}</a></p>
          <p><a href="{{account.web}}">{{ account.web }}</a></p>
        </td>
        <td width="42%" valign="top">
          <h2 class="label border-first">{{labels.to}}</h2>
          <h3 class="txt">{{ contact.full_name }}</h3>
          <p>{% if contact.tax_id != blank %} {{labels.tax_id}}: {{contact.tax_id}} {% endif %}</p>
          <p>{% if contact.vat_number != blank %} {{labels.vat_number}}: {{contact.vat_number}} {% endif %}</p>
          <p>{{ contact.contact_name }}</p>
          <address>{{ contact.formatted_address | newline_to_br }}</address>
        </td>
      </tr>
      <tr>
        <td colspan="2" valign="top">
        <table id="items" width="100%" border="0" cellspacing="0" cellpadding="0">
          <thead>
            <tr class="border-first">
              <th scope="col" class="left">{{labels.description}}</th>
              <th width="18%" class="right">{{labels.quantity}}</th>
              <th width="20%" class="right">{{labels.unit_price}}</th>
              <th width="20%" class="right">{{labels.amount}}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <tr>
        ...
        <td colspan="2" valign="top">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td width="47%" valign="top" {% if document.payment_details != blank %} class="space12" {% endif %}>
                <h2 class="label border-first">{{labels.notes}}</h2>
                <p class="smalltxt">Momentum is the trading name of Brandvee Ltd</p>

                {% if document.notes != blank %}
                <p class="smalltxt">{{document.notes | textilize}}</p>
                {% endif %}
              </td>
              {% if document.payment_details != blank %}
              <td width="53%" valign="top">
                <h2 class="label border-first">{{labels.payment_details}}</h2>
                <p class="smalltxt">{{document.payment_details | textilize}}</p>
              </td>
              {% endif %}
            </tr>
          </table>
