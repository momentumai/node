    ...
    return Promise.all([
        ...
        db.q(q['properties'], {
            ...
            return {
                ...
                'real_team_id': teamId,
                'guide': guide,
                'fb_access_token': rows[0].fb_access_token
            };
        })
    ]).then(function (res) {
        if (res[0]) {
...
auth.getFilters = function (teamId, userId, pool) {
    return auth.getTeamUserData(
        ...
        if (data) {
            ...
            parsed[0] = parsed[0] || 'NONE';
            parsed[1] = parsed[1] || 'NONE';
            parsed[2] = parsed[2] || 'NONE';
        }

        parsed = parsed.map(function (cat) {
            return db.escape(cat);
        });
