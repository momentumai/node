...
facebook.crawlerValidate = function (accessToken, type, asset) {
    return validatePermissions(accessToken).then(function (perms) {
        if (perms !== 'valid') {
            return 0;
        }

        if (type === 1) {
            return facebook.validatePages(asset, accessToken);
        }

        return validateAdAccount(accessToken, asset);
    });
};
